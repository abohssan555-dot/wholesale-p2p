...
import { Store, Upload, FileText, X, Loader2, CheckCircle2, Clock, XCircle, Package } from "lucide-react";
...
// المستندات المطلوبة لكل نوع مقدّم طلب — التاجر هو الافتراضي هنا
const REQUIRED_DOCS = {
  trader: ["السجل التجاري", "رخصة بلدية"],
  business_customer: ["السجل التجاري", "هوية المفوّض"],
  driver: ["رخصة القيادة", "استمارة المركبة"],
};
      ...
      <div className="w-full max-w-md">
        <div className="flex items-center gap-2 mb-6 justify-center">
          ...
          <div className="text-center">
            ...
            <div className="text-[11px]" style={{ color: T.sub }}>تسجيل تاجر جديد</div>
          </div>
        </div>
        {children}
      </div>
        ...
        <React.Fragment key={l}>
          <div className="flex items-center gap-1.5">
            <div
              ...
              style={{
                background: i <= step ? T.seal : T.paperDeep,
                color: i <= step ? T.ink : T.sub,
              }}
            >
              {i + 1}
            </div>
...
function AccountStep({ onDone }) {
  ...
  const submit = async (e) => {
    ...
    if (!data.session) {
      setNeedsConfirm(true); // مشروع Supabase مفعّل عنده تأكيد البريد
      return;
    }
    onDone();
  };
  ...
  return (
    <Card>
      <form onSubmit={submit}>
        <Field label="اسم صاحب المتجر / المنشأة" required value={form.full_name} onChange={set("full_name")} />
...
/* -------------------- Step 1: Documents -------------------- */
function DocumentsStep({ session, onDone }) {
  const required = REQUIRED_DOCS.trader;
  ...
  const submit = async () => {
    setErr("");
    const missing = required.filter((l) => !files[l]);
    if (missing.length) {
...
function StatusStep({ session }) {
  ...
  const stageMeta = {
    ...
    approved: { icon: CheckCircle2, label: "تم اعتماد حسابك! تقدر تدخل متجرك الآن", tone: T.good },
    ...
    <Card>
      <div className="text-center py-4">
        ...
        <div className="text-sm font-medium mb-1" style={{ color: T.ink }}>
          {meta?.label || "جارٍ التحميل..."}
        </div>
        <div className="text-xs" style={{ color: T.sub }}>
          راح نبلغك بأي تحديث. تقدر تسكّر الصفحة وترجع لاحقاً تتأكد من الحالة.
        </div>
      </div>
    </Card>
  );
...
export default function TraderOnboarding() {
  ...
  return (
    <Shell>
      ...
      {step === 1 && session && <DocumentsStep session={session} onDone={() => setStep(2)} />}
